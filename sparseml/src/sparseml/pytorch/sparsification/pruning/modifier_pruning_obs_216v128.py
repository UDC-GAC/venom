...
__all__ = [
    "OBSnm16v128PruningModifier",
    "OBSnm16v128PruningParamsScorer",
]
...
@PyTorchModifierYAML()
class OBSnm16v128PruningModifier(BaseGradualPruningModifier):
    """
    ...
    def __init__(
        ...
        #self.init_sparsity = 2/12
        #self.final_sparsity = 2/12
    ...
    def _get_scorer(self, params: List[Parameter]) -> PruningParamsGradScorer:
        ...
        """
        return OBSnm16v128PruningParamsScorer(
            params=params,
...
class OBSnm16v128PruningParamsScorer(PruningParamsGradScorer):
    """
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        # Change depending on your n:m pattern and V value
        n = 14
        m = 16
        v = 128
        m_p = 4 #! Ampere architecture fixed value

        if self._is_main_proc:
            print("len(self._finvs): ", len(self._finvs))
            for i, finv in enumerate(self._finvs):
                block_w = self._params[i].data.view(-1, m).to(finv.dev)  # (d/8, 8)
                block_finv = (
                    torch.cat(
                        [
                ...
                print("finv.f_inv: ", finv.f_inv.shape, ", block_finv.shape: ", block_finv.shape)
                print("block_w: ", block_w.shape, ", params.data: ", self._params[i].data.shape)
                # n:m selection
                vector_comb = comb(m_p,m-n)
                cols_comb = comb(m, m_p)

                e_nm = torch.stack([torch.ones((vector_comb, m), dtype=torch.long, device=block_finv.device)] * cols_comb)
                nrows, ncols = self._params[i].shape
                print("self._params[i].shape: ", self._params[i].shape, i)

                col_comb = 0
                #FIXME: Generate masks on the fly, do not store all in memory
                for i2 in range(m):
                    for j in range(i2+1, m):
                        for k in range(j+1, m):
                            for w in range(k+1, m):
                                #print(i2,j,k,w)

                                opts = [i2,j,k,w]
                                vec_comb = 0
                                for ii in range(m_p):
                                    for jj in range(ii+1, m_p):
                                        e_nm[col_comb][vec_comb,opts[ii]] = 0
                                        e_nm[col_comb][vec_comb,opts[jj]] = 0
                                        vec_comb+=1

                                col_comb+=1
                num_combs = vector_comb

                best_comb = torch.ones((nrows//v, ncols//m), device=block_w.device)*torch.inf
                best_cols = torch.zeros((nrows//v, ncols//m), dtype=torch.long, device=block_w.device)
                global_best_to_prune_nm_idx = torch.zeros(nrows*ncols//m, dtype=torch.long, device=block_w.device)
                best_block_finv_w_nm = torch.ones((num_combs, block_w.shape[0], n), device=block_w.device)

                # batched outer products to select from 8x8 correlations matrix
                #for pos, cm in enumerate(range(1)):
                for pos, cm in enumerate(range(cols_comb)):
                    print("layer ", i, " at comb: ", pos)
                    ee_nm = torch.einsum("bij,bkj->bik", e_nm[cm].view(num_combs,m,1), e_nm[cm].view(num_combs, m, 1))
                    ...
                    for k in range(num_combs):
                        block_w_nm = torch.masked_select(block_w, e_nm[cm][k].unsqueeze_(0).bool()).view(-1, n)
                    ...
                    a = scores_nm.T.gather(1, best_to_prune_nm_idx.unsqueeze(1)).view(-1, ncols//m)
                    #print("split problem")
                    val = torch.stack([torch.sum(s,0) for s in torch.split(a, v)])

                    lt = torch.lt(val,best_comb)
                    best_comb = torch.where(lt, val, best_comb)
                    best_cols = torch.where(lt, torch.ones((nrows//v, ncols//m), device=block_w.device)*pos, best_cols)

                    #print("repeat problem")
                    w = lt.unsqueeze(1).repeat(1,v,1).flatten()
                    best_block_finv_w_nm[:, w, :] = block_finv_w_nm[:, w, :]

                    global_best_to_prune_nm_idx= torch.where(w, best_to_prune_nm_idx, global_best_to_prune_nm_idx)

                global_best_to_prune_nm_idx = global_best_to_prune_nm_idx.to(torch.long)
                block_finv_w_flatten = torch.stack([best_block_finv_w_nm[global_best_to_prune_nm_idx[j], j, :] for j in range(global_best_to_prune_nm_idx.shape[0])]).flatten()

                best_cols = best_cols.to(torch.long)
                best_cols = best_cols.unsqueeze(1).repeat(1,v,1).view(global_best_to_prune_nm_idx.shape).flatten()

                block_finv_w[i] = torch.stack([e_nm[col][idx] for col,idx in zip(best_cols,global_best_to_prune_nm_idx)]).float()

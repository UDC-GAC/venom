...
class OBSPruningModifier(BaseGradualPruningModifier):
    ...
    Supported mask types: unstructured and block8.
    ...
    :param mask_type: String to define type of sparsity to apply. 'unstructured'
        and 'block8' are supported. Default is 'unstructured'
    ...
    def __init__(
        ...
        self._supported_masks = ("unstructured", "block8")
    ...
    def _validate(self):
        ...
        if self._mask_type == "block8" and self._fisher_block_size % 8 != 0:
            raise ValueError(
                "fisher_block_size must be divisible by 8 for block8 pruning"
            )
...
class OBSPruningParamsScorer(PruningParamsGradScorer):
    ...
    framework. Implements unstructured and semi-structured (block8) scoring and
    pruning.
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        if self._is_main_proc:
            for i, finv in enumerate(self._finvs):
                ...
                else:  # self._mask_type == "block8":
                    block_w = self._params[i].data.view(-1, 8).to(finv.dev)  # (d/Q, Q)
                    block_finv = (
                        torch.cat(
                            [
                                finv.f_inv[:, i : i + 8, i : i + 8]
                                for i in range(0, finv.B, 8)
                            ],
                            dim=1,
                        )
                        .reshape((finv.d // finv.B, finv.B // 8, 8, 8))
                        .reshape((finv.d // 8, 8, 8))
                    )  # (Q, d/Q, Q) -> (d/Q, Q, Q)

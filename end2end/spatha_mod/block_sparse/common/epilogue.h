...
//if(block_M==64 && warp_M==64){
    ...
    int global_offset = idx_block_M*block_M*N + idx_block_N*block_N + idx_warp_M*warp_M*N + ((lane>>3)<<2)*N + ((lane&7)<<3);
    half *D_tile = C + global_offset;
    ...
    if (beta == 0.0f) {
        ...
        for(int m=0; m<iter_copy_C; m++){
            ...
            //#pragma unroll
            for(int b=0; b<4; b++){
                const half *src21 = src2+ldc*b;
                HalfVector<8> buffer;

                buffer.ld(src21);
                for (int i = 0; i < 4; i++){
                    buffer.x[i] = __hmul2(buffer.x[i], aa);
                }
                __half2 *dst2 = (__half2*)&out[b*4];
                for (int i = 0; i < 4; i++) {
                    dst2[i] = __hfma2(dst2[i], bb, buffer.x[i]);
                }

                *(float4*)(dst+ldc*b) = *(float4*)(&out[b*4]);
            }
...
/*
    ...
    half *D_tile = C + idx_block_M * block_M * ldc + idx_block_N * block_N;
    //HalfVector<8> buffer;

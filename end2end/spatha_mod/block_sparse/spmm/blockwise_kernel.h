    ...
    const half *A_values, const uint *A_indices, const half *B, half *shared_mem_workspace, float alpha, half *C, float beta, half *D)
{
    ...
    half *D_tile = D+idx_block_M*block_M+idx_warp_M*warp_M+(lane%4)*2;//+idx_block_N*block_N+idx_warp_N*warp_N+(lane%4)*2;
    ...
    for (int i = 0; i < iter_copy_C; i++) {
        ...
        *((float*)(&buffer2[0])) = *((float*)(src2));
        ...
        /* for (int i = 0; i < 2; i++){
            dst2[i] = __hmul2(dst2[i], aa);
        } */
        /* if(!blockIdx.x && !blockIdx.y && !threadIdx.x){
            printf("%.2f \n", __half2float(buffer[0]));
        } */
        ...
        *((float2*)(dst)) = *((float2*)&src[0]);
        ...
        //*((float2*)(&buffer[0])) = *((float2*)(src2+1));
        /* for (int i = 0; i < 2; i++){
            buffer[i] = __hmul2(buffer[i], aa);
        } */
